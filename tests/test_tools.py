"""Offline tests: one per Sonarr v3 endpoint, plus error-path tests.

No network. The list of operations is generated from the vendored spec at
tests/data/sonarr_openapi.json (same skip rules as the authoring script), and
...
SPEC_PATH = os.path.join(os.path.dirname(__file__), "data", "sonarr_openapi.json")
...
def registry_for(method, path):
    for spec in sonarr_mcp._TOOL_REGISTRY:
...
async def server(recorder, monkeypatch):
    ...
    client = sonarr_mcp.build_client("http://sonarr.example.com", "test-key", transport=transport)
    monkeypatch.setattr(sonarr_mcp, "_client", client)
    yield sonarr_mcp.mcp
    await client.aclose()


_OP_GROUP = {op: group for group, ops in sonarr_mcp._GROUPS.items() for op in ops}
...
def test_registry_covers_spec_exactly():
    ...
    registry_ops = {(s["method"], s["path"]) for s in sonarr_mcp._TOOL_REGISTRY}
    assert registry_ops == spec_ops_set


def test_all_registered_tools_have_unique_names():
    names = [s["name"] for s in sonarr_mcp._TOOL_REGISTRY]
...
def test_all_registry_names_grouped():
    ...
    registry_names = [s["name"] for s in sonarr_mcp._TOOL_REGISTRY]
    grouped_names = [n for names in sonarr_mcp._GROUPS.values() for n in names]
...
async def test_group_tools_are_the_only_registered_tools(server):
    ...
    assert {t.name for t in tools} == set(sonarr_mcp._GROUPS)
...
async def test_unknown_operation_rejected_by_schema(server):
    ...
    with pytest.raises(ToolError, match="validation error"):
        async with Client(server) as c:
            await c.call_tool("sonarr_tags", {"operation": "not_a_real_operation"})


# --- query params --------------------------------------------------------------

async def test_query_params_use_wire_names(server, recorder):
    await call(server, "sonarr_list_history", page=2, page_size=50, sort_key="date", sort_direction="descending")
...
async def test_list_params_serialize_repeatedly(server, recorder):
    await call(server, "sonarr_list_episodefile", episode_file_ids=[1, 2, 3])
    assert recorder.params.get_list("episodeFileIds") == ["1", "2", "3"]


async def test_empty_optional_params_are_omitted(server, recorder):
    await call(server, "sonarr_list_series")
    assert "tvdbId" not in recorder.params
    assert "includeSeasonImages" in recorder.params  # bool False is sent explicitly


async def test_lookup_and_path_id_encoding(server, recorder):
    await call(server, "sonarr_lookup_series", term="Breaking Bad")
    assert recorder.url.path == "/api/v3/series/lookup"
    assert recorder.params["term"] == "Breaking Bad"

    await call(server, "sonarr_get_series", id=42)
    assert recorder.url.path == "/api/v3/series/42"


# --- request bodies ------------------------------------------------------------

async def test_body_sent_as_json(server, recorder):
    body = {"title": "Breaking Bad", "qualityProfileId": 1, "rootFolderPath": "/m/series", "tvdbId": 81189}
    await call(server, "sonarr_add_series", body=body)
    assert recorder.json == body


async def test_array_body_sent_as_json(server, recorder):
    body = [{"id": 1, "monitored": False}]
    await call(server, "sonarr_bulk_update_episodefile", body=body)
    assert recorder.json == body
...
async def test_api_key_sent_as_x_api_key_header(server, recorder):
    await call(server, "sonarr_list_tag")
    assert recorder.headers["x-api-key"] == "test-key"


async def test_no_api_key_means_no_auth_header(recorder, monkeypatch):
    transport = httpx.MockTransport(recorder.handler)
    client = sonarr_mcp.build_client("http://sonarr.example.com", None, transport=transport)
    monkeypatch.setattr(sonarr_mcp, "_client", client)
    await call(sonarr_mcp.mcp, "sonarr_list_tag")
...
async def test_404_error_message_reaches_caller(server, recorder):
    recorder.response = httpx.Response(404, json={"message": "Series not found"})
    with pytest.raises(ToolError, match="Series not found"):
        await call(server, "sonarr_get_series", id=999999)
...
async def test_400_error_surfaces_status(server, recorder):
    ...
    with pytest.raises(ToolError, match="400"):
        await call(server, "sonarr_add_series", body={})
...
def test_main_requires_sonarr_url(monkeypatch):
