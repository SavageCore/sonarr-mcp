...
- Push main and the tag:
  ...
  ```
- This server is not yet deployed to the Proxmox host or the christopfarr project copy. When it is, follow the pattern in the other `-mcp` servers: push tags, sync the project copy, then `ssh root@192.168.50.3 -- 'cd /root/sonarr-mcp && git fetch origin && git reset --hard origin/main && uv tool install --force .'`.
